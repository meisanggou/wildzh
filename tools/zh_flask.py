...
class _JYFlask(Flask):
    ...
    def run(self, host=None, port=None, debug=None, **options):
        self.run_time = datetime.now().strftime(JYFlask.TIME_FORMAT)
...
class JYFlask(_JYFlask):
    ...
    def _assign_default_g():
        pass
        # g.requests.headers["X-JY-FROM"] = "JY-API-V2"

    @staticmethod
    def _request_real_ip():
        request_ip = request.remote_addr
        if "X-Forwarded-For" in request.headers and request_ip in JYFlask.TRUST_PROXY:
            tracert_ip = request.headers["X-Forwarded-For"].split(",")
            request_ip = tracert_ip[0]
            if isinstance(JYFlask.TRUST_PROXY, list):
                for i in range(len(tracert_ip) - 1, -1, -1):
                    one_proxy = tracert_ip[i].strip()
                    if one_proxy not in JYFlask.TRUST_PROXY:
                        request_ip = one_proxy
                        break
        g.request_IP = JYFlask.ip_man.ip_value_str(ip_str=request_ip)
        if g.request_IP == 0:
    ...
    def __init__(self, import_name, **kwargs):
        ...
        kwargs.pop("db", None)
        self.blues = []
        self.skip_auth = kwargs.pop("skip_auth", False)
        super(JYFlask, self).__init__(import_name, **kwargs)
