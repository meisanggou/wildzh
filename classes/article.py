...
class ArticleManager(object):
    ...
    def insert_info(self, article_type, article_no, author, title, abstract, adder=None, article_desc=None,
                    pic_url=None):
        kwargs = dict(article_no=article_no, author=author, title=title, abstract=abstract, update_time=time(),
                      article_desc=article_desc, pic_url=pic_url, adder=adder, article_type=article_type)
    ...
    def new_article(self, article_type, author, title, abstract, content, adder=None, article_desc=None, pic_url=None):
        article_no = uuid.uuid1().hex
        l = self.insert_info(article_type, article_no, author, title, abstract, adder, article_desc, pic_url)
    ...
    def _update_info(self, article_type, article_no, update_value):
        ...
        l = self.db.execute_update(self.t_info, where_value=where_value, update_value=update_value)
        return l

    def _update_status(self, article_type, article_no, add_status=None, sub_status=None):
        where_value = dict(article_no=article_no, article_type=article_type)
    ...
    def update_article(self, article_type, article_no, author=None, title=None, abstract=None, content=None,
                       article_desc=None, pic_url=None):
    ...
    def _select_info(self, article_type, article_no, where_cond=None, where_cond_args=None):
        cols = ["article_no", "author", "adder", "title", "article_desc", "abstract", "pic_url",
                "update_time", "status", "article_type"]
        where_value = dict()
    ...
    def query_article(self, **kwargs):
        ...
        db_items = self._select_info(article_type, article_no, where_cond, where_cond_args)
