...
Page({
  ...
  onLoad: function(options) {
    ...
    if (!("exam_no" in options)) {
      ...
      that.setData({
        exam_no: options["exam_no"],
        examName: options["exam_name"]
      })
      wx.request2({
        url: '/exam/questions/?exam_no=' + options["exam_no"] + "&num=20",
        method: 'GET',
        success: res => {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          var test_id = app.globalData.testIdPrefix + that.data.exam_no + "_" + timestamp;
          ...
          wx.redirectTo({
            url: '../result/result?exam_no=' + that.data.exam_no + "&timestamp=" + timestamp
          })
        }
      }
    })
